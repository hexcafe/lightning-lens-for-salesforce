...
const DEV_TOOLS_ICON_SVG: string = `
...
export function initializeFloatingButton(shadowRoot: ShadowRoot): void {
  let isDragging: boolean = false;
  let position: Position = {
    x: window.innerWidth - 80,
    y: window.innerHeight - 80,
  };
  let dragStart: DragStart = { x: 0, y: 0, initialX: 0, initialY: 0 };

  // Create the floating button
  const button = document.createElement("button");
  button.className = "sfdc-companion-button";
  button.innerHTML = DEV_TOOLS_ICON_SVG;
  button.style.left = `${position.x}px`;
  button.style.top = `${position.y}px`;

  // Create the modal elements (initially hidden)
  const modalOverlay = document.createElement("div");
  modalOverlay.className = "sfdc-companion-modal-overlay";
  ...
  modalOverlay.innerHTML = `
    <div class="sfdc-companion-modal-content">
        ...
        <main class="sfdc-companion-modal-body">
            <iframe allow="clipboard-read; clipboard-write" src="${chrome.runtime.getURL("/index.html")}"></iframe>
        </main>
    </div>
  `;
  ...
  const modalContent = modalOverlay.querySelector<HTMLDivElement>(
    ".sfdc-companion-modal-content",
  );
  const closeButton =
    modalOverlay.querySelector<HTMLButtonElement>(".close-btn");

  if (!modalContent || !closeButton) {
    console.error("Could not find modal content or close button.");
    return;
  }

  // --- State Management & Event Logic ---
  ...
  window.addEventListener("mousemove", (e: MouseEvent) => {
    ...
    position.x = dragStart.initialX + dx;
    position.y = dragStart.initialY + dy;
    button.style.left = `${position.x}px`;
    button.style.top = `${position.y}px`;
  });
  ...
  window.addEventListener("mouseup", () => {
    if (isDragging) {
      ...
      // By removing setTimeout, isDragging is set to false synchronously
      // before the click event can fire.
      isDragging = false;
    }
  });
  ...
  modalContent.addEventListener("click", (e: MouseEvent) =>
    e.stopPropagation(),
  );
}
...
hostElement.id = "salesforce-devtools-companion-host";
document.body.appendChild(hostElement);

// 2. Create the Shadow DOM root
const shadowRoot = hostElement.attachShadow({ mode: "open" });

// 3. Inject the CSS styles into the Shadow DOM
const styleElement = document.createElement("style");
styleElement.innerHTML = styles;
shadowRoot.appendChild(styleElement);

// 4. Initialize our vanilla JavaScript UI inside the Shadow DOM
initializeFloatingButton(shadowRoot);
